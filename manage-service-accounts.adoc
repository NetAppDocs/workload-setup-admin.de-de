...
---
= Servicekonten in NetApp Workload Factory verwalten
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Optional können Sie ein Zugriffstoken für die Auth0-Management-API durch einen Client-Credentials-Austausch erhalten. Weitere Informationen zum Generieren eines Aktualisierungstokens finden Sie unter link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console API-Dokumentation"^].
+
Das curl-Beispiel zeigt, wie Sie die Client-ID und das Geheimnis nehmen und eine API verwenden können, um das Zugriffstoken zu generieren, das zeitlich begrenzt ist. Das Token bietet mehrere Stunden Zugriff auf die NetApp Workload Factory APIs.
...
----
+
Senden Sie die Ausgabe an https://netapp-cloud-account.auth0.com/oauth/token, um das Zugriffstoken zu erhalten.
