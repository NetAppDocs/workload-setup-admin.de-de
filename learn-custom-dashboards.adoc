...
Workload Factory benötigt einen Link, um eine Verbindung zu FSx for ONTAP Dateisystemen herzustellen. Ein Link schafft eine Vertrauensbeziehung und Konnektivität zwischen Workload Factory und einem oder mehreren FSx for ONTAP Dateisystemen. Dadurch ist es möglich, bestimmte Dateisystemfunktionen direkt über die ONTAP REST API-Aufrufe zu überwachen und zu verwalten, die über die Amazon FSx for ONTAP API nicht verfügbar sind.
