...
NetApp Workload Factory ist für Benutzer als Cloud-Service über die webbasierte Konsole zugänglich.  Bevor Sie beginnen, sollten Sie Folgendes verstehen:link:workload-factory-overview.html["Workload Factory"] Die
...
[role="quick-margin-para"]
Dieser Schritt ist optional. Workload Factory kann ohne die Hinzufügung von Anmeldeinformationen für den Zugriff auf Ihr AWS-Konto verwendet werden. Das Hinzufügen von AWS-Anmeldeinformationen zu Workload Factory gibt Ihrem Workload Factory-Konto die erforderlichen Berechtigungen, um FSx for ONTAP Dateisysteme zu erstellen und zu verwalten sowie bestimmte Workloads wie Datenbanken bereitzustellen und zu verwalten.
